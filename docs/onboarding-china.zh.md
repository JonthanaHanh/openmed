...
Hugging Face 模型镜像、可迁移的本地缓存，以及面向个人信息保护的保守策略起点。
...
OpenMed 目前尚未提供 `china_pipl` 策略。现在可以把内置的 `strict_no_leak`
作为保守起点，并明确保留未来的替换位置：

```python
from openmed.core.policy import list_policies, load_policy

# 仅当 list_policies() 已包含 "china_pipl" 时，才把此值改为它。
...
这段代码完全从已安装的软件包中加载
`openmed/core/policies/strict_no_leak.json`，不依赖模型或网络。
在常规去标识化调用中，可以把同一个 `policy_name` 传给
`deidentify(synthetic_text, policy=policy_name)`。在生产环境处理个人信息或
受保护健康信息 (PHI) 前，请重新评估技术策略和法律治理措施。
