...
transferable local cache, and a conservative privacy-policy starting point.
...
OpenMed does not yet ship a `china_pipl` profile. Use the bundled
`strict_no_leak` profile as a conservative current baseline and make the future
swap-in point explicit:

```python
from openmed.core.policy import list_policies, load_policy

# Change this one value to "china_pipl" only after it appears in list_policies().
...
`openmed/core/policies/strict_no_leak.json`; it does not need a model or network
connection. Pass the same `policy_name` to the normal runtime call as
`deidentify(synthetic_text, policy=policy_name)`. Reassess the policy and your
legal controls before production processing of personal information or
