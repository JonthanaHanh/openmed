...
| Multilingual PII | 26 supported PII language codes: ar, as, bn, de, en, es, fr, gu, he, hi, id, it, ja, kn, ko, ml, mr, nl, or, pa, pt, ro, ta, te, th, and tr; 17 retain bundled or manifest-backed defaults and the 11 Indic languages can use an explicitly configured optional adapter. Includes locale validators, script detection, date/number normalization, deterministic locale PHI generation, and ID-only national-ID providers for additional locales. | `openmed/core/pii_i18n.py`, `openmed/core/script_detect.py`, `openmed/core/locale_formats.py`, `openmed/core/anonymizer/locales.py`, `openmed/training/synthetic/locale_phi.py`, `examples/pii_multilingual_new_languages.py` |
