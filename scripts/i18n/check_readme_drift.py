#!/usr/bin/env python3
"""Detect source README changes that have not been reviewed in Hindi."""
...
DEFAULT_TRANSLATIONS = (Path("README.hi.md"),)
...
def build_manifest(root: Path, manifest_path: Path) -> dict[str, object]:
    ...
    if not (root / GLOSSARY).is_file():
        raise DriftError(f"Hindi README glossary is missing: {GLOSSARY}")
