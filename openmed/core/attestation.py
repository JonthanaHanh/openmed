...
def _safe_offline_assertion(value: Any) -> dict[str, Any]:
    source = value if isinstance(value, Mapping) else offline_mode_assertion()
    ...
    return {
        "asserted": bool(source.get("asserted", False)),
        "local_only": bool(source.get("local_only", False)),
        "source": str(source.get("source", "none")),
        "network_guard_requested": bool(source.get("network_guard_requested", False)),
        "dependency_flags_enabled": bool(source.get("dependency_flags_enabled", False)),
        "environment_flags": {
