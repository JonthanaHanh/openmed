...
def _reject_unknown_fields(
    ...
    if unknown:
        raise ValueError(
            f"{object_name} contains unsupported fields: {', '.join(unknown)}"
        )
...
class AuditChain:
    ...
    def __init__(
        ...
        *,
        ...
        self._declared_head_hash = declared_head_hash or current_head
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "AuditChain":
        ...
        if data.get("version") != CHAIN_VERSION:
            raise ValueError(
                f"unsupported audit chain version: {data.get('version')!r}"
            )
