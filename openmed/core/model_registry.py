    ...
    *,
    ...
    if languages is None:
        ...
        normalized_languages = tuple(
            sorted({str(language).strip().casefold() for language in languages})
        )
...
def _configured_indic_pii_models(lang: str) -> Dict[str, ModelInfo]:
    config = get_indic_encoder_config()
...
def get_pii_models_by_language(lang: str) -> Dict[str, ModelInfo]:
    """Return PII models plus an explicitly configured Indic encoder adapter."""
    if lang == "en":
        ...
        if not language_models:
            ...
            if default_model_id:
                # Internal fallback: DEFAULT_PII_MODELS is the validated source of
                # truth, so language-pack callers can reuse multilingual privacy
                # filters safely.
                language_models = {
                    key: info
                    for key, info in OPENMED_MODELS.items()
    ...
    language_models.update(_configured_indic_pii_models(lang))
