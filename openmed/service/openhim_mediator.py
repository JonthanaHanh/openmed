...
class OpenHIMMediatorSettings:
    ...
    def from_env(cls) -> "OpenHIMMediatorSettings":
        ...
        if not api_url.startswith(("http://", "https://")):
            raise OpenHIMMediatorConfigurationError(
                "OPENMED_OPENHIM_CORE_URL must start with http:// or https://"
            )
...
def mediator_response_headers(headers: Mapping[str, str]) -> dict[str, str]:
    """Copy response-safe headers without credentials or hop-by-hop fields."""

    return {
        str(key): str(value)
        for key, value in headers.items()
        if str(key).lower() not in _RESPONSE_HEADER_DENYLIST
    }
    ...
    *,
    ...
    if media_type == "application/fhir+json" or media_type in {
        ...
        if resource_type == "Bundle":
            ...
            transformed = de_identify(payload, deidentifier=deidentifier)
