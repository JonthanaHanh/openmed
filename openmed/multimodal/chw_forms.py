    ...
    if _is_metadata_path(parts):
        ...
        if label is not None:
            ...
            decision = ChwFieldDecision(
                ...
                assigned_class=SAFE,
                action=ACTION_KEEP,
                detection_source="coded_or_safe",
            )
...
def _redact_csv_export(
    ...
    *,
    ...
    width = len(headers)
    rows = [tuple(row) + ("",) * max(0, width - len(row)) for row in records[1:]]
...
def _apply_action(
    ...
    *,
    ...
    if decision.action == ACTION_HASH:
        ...
        redacted = _redact_free_text(str(value), text_redactor=text_redactor)
...
def _shift_date_value(
    value: str, label: str, *, fixed_days: int | None, lang: str
) -> str:
    ...
    if days is None:
        ...
        days = int.from_bytes(digest[:2], "big") % 730 - 365
...
def _redact_free_text(value: str, *, text_redactor: TextRedactor | None) -> str:
