...
def _read_value_set(resource: Mapping[str, Any]) -> _ValueSet:
    ...
    if isinstance(expansion, Mapping) and isinstance(expansion.get("contains"), list):
        ...
        _collect_expansion_codes(expansion["contains"], codes, inherited_system="")
        return _ValueSet(codes=frozenset(codes), complete=True)
    ...
    for include in includes:
        ...
        system_value = system if isinstance(system, str) else ""
        for concept in concepts:
            if isinstance(concept, Mapping) and isinstance(concept.get("code"), str):
                codes.add((system_value, concept["code"]))
    ...
    for system, code in candidates:
        if system:
            if (system, code) in codes:
                return True
        elif any(candidate_code == code for _, candidate_code in codes):
            return True
    return False
...
def _check_slice(
    ...
    discriminators = _slice_discriminators(slice_element, descendants, str(base_path))
...
def _slice_discriminators(
    ...
    for _, expected in _fixed_constraints(slice_element):
        constraints.append(((), "fixed", expected))
    for _, expected in _pattern_constraints(slice_element):
        constraints.append(((), "pattern", expected))
    for descendant in descendants:
        relative = _descendant_segments(descendant.get("path"), base_path)
        if relative is None:
            continue
        for _, expected in _fixed_constraints(descendant):
            constraints.append((relative, "fixed", expected))
        for _, expected in _pattern_constraints(descendant):
            constraints.append((relative, "pattern", expected))
    return constraints
