...
def _fake_prefetch_model(model: str, *, cache_dir: str) -> str:
    ...
    (snapshot / "model.safetensors").symlink_to("../../blobs/model-data")
    (repo_dir / "refs").mkdir()
...
def test_manifest_counts_symlink_itself_without_double_counting_target(
    ...
    assert symlink["link_target"] == os.readlink(link_path)
    assert symlink["size_bytes"] == link_path.lstat().st_size
