...
def _enabled_settings(registration: dict[str, Any]) -> OpenHIMMediatorSettings:
...
def test_recorded_registration_and_heartbeat_handshake_is_idempotent() -> None:
    ...
    assert observed[1]["authorization"].startswith("Basic ")
    assert observed[1]["body"] == registration
    assert observed[2]["body"]["config"] is True
    assert observed[2]["body"]["uptime"] == 5.0
