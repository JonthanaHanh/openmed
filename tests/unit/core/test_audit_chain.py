...
def test_loading_rejects_unknown_fields_that_could_carry_phi() -> None:
    payload = json.loads(_three_entry_chain().to_json())
    payload["entries"][0]["patient_name"] = "Alice Rivera"

    with pytest.raises(ValueError, match="unsupported fields: patient_name"):
