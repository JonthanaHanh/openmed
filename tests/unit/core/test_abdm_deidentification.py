...
def test_abdm_recognizer_rejects_invalid_checksum_numbers() -> None:
    text = "ABHA 11328941304001 and Aadhaar 655227804686"
...
def test_generated_abha_and_aadhaar_pass_checksum_validators() -> None:
    faker = Faker("en_IN")
    register_clinical_providers(faker)
    faker.seed_instance(672)
...
def test_india_dpdp_policy_contract_replaces_every_abdm_identifier_label() -> None:
    # OM-671 owns bundled-profile registration. Exercise OM-672's integration
    # boundary independently: every new source label must resolve through the
    # India profile's ID_NUM=replace action rather than fall through to OTHER.
    base = load_policy("hipaa_safe_harbor")
    profile = replace(
        base,
        name="india_dpdp_act",
        actions={**base.actions, ID_NUM: "replace"},
    )
