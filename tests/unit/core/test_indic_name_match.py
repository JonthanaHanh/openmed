...
def test_vault_links_variants_and_renders_one_identity_per_script(tmp_path):
    ...
    identities = {
        vault.get(source, label="PERSON", lang="hi")
        for source in ("संजय", "Sanjay", "Sanjai")
    }
    assert identities == {"Ketan Sharma"}
...
def test_disabled_vault_keeps_legacy_exact_surface_keys():
...
def test_deidentify_config_enables_code_mixed_vault_matching(monkeypatch):
    ...
    assert len(vault.entries()) == 1
    assert (
        len({vault.get(surface, label="PERSON", lang="hi") for surface in variants})
        == 1
    )
