...
def resolve_model_route(text: str, language_override: str = "auto") -> ModelRoute:
    ...
    # Chinese uses its dedicated checkpoint. ``en`` keeps OpenMed's currently
    # supported language-specific pattern pack while the checkpoint handles Han.
    openmed_language = "hi" if language == "hi" else "en"
    return ModelRoute(
        language=language,
        model_id=MODEL_IDS[language],
        openmed_language=openmed_language,
    )
